    ...
    public class ExamMongoDbSeeding
    {
               ...
               ILogger<ExamMongoDbSeeding> logger)
        {
            ...
            await policy.ExecuteAsync(async () =>
            {
                ...
                if (await database.GetCollection<Exams>(Constants.Collections.Exam).EstimatedDocumentCountAsync() ==
                    0)
                {
                    await database.GetCollection<Exams>(Constants.Collections.Exam)
                        .InsertManyAsync(GetPredefinedExams(categoryId1));
                }
            });
        }

        private List<Exams> GetPredefinedExams(string categoryId1)
        {
            return new List<Exams>()
            {
                new Exams("Exam 1", "Lorem Ipsum is simply dummy text of the printing and typesetting industry",
                    ...
                    4,
                    true),
                new Exams("Exam 2", "Lorem Ipsum is simply dummy text of the printing and typesetting industry",
                    "<p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.</p>",
                    5,
